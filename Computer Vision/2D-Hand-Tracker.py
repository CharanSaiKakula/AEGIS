...
class HandTracker:
    def __init__(self, camera_id: int = 0):
        ...
        self.camera = cv2.VideoCapture(camera_id) #replace camera id with tello stream
        ...
        # MediaPipe drawing utilities for visualizing hand landmarks
        self.mp_drawing = mp.solutions.drawing_utils

        # Pre-calculate frame center coordinates for offset calculations
        self.frame_width = 640
        self.frame_height = 480
        self.center_x = self.frame_width // 2  # "//" means integer division with flooring
        self.center_y = self.frame_height // 2  

    def get_hand_data(self) -> Tuple[Optional[np.ndarray], HandData]:
        """
        Capture a frame from the camera and detect hand/palm.
        ...
        # If frame capture failed (e.g., camera disconnected), return defaults
        if not ret:
    ...
    def release(self):
        """Clean up resources: close camera and MediaPipe."""
        self.camera.release()  # Release camera
        self.mp_hands.close()  # Close MediaPipe hands solution
...
def main():
    ...
    print("Position your palm in front of the camera for detection.")
